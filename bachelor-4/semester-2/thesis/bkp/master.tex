...
даётся как
\begin{equation}
\psi_k\ket{0}=0,\quad k\le 0;\qquad\psi^* \ket{0}=0,\quad
k> 0
.\end{equation} 
...
\begin{equation}
\psi_k\ket{m}=0,\quad k\le m;\qquad\psi^* \ket{m}=0,\quad
k> m
,\end{equation}
и состояния
\[
	\ket{m,\,\lambda}= \psi_{\lambda_1+m}\psi^*_m\psi_{\lambda_2+m-1}\psi^*_{m-1}\cdots\psi_{\lambda_{l(\lambda)}+m-l(\lambda)}\psi^*_{m-l(\lambda)}\ket{m}
.\] 
