...
y, x = np.loadtxt('data5.csv', delimiter=';', unpack=True)
...
#plt.legend()
plt.title(r'ВАХ тиратрона для напряжения накала $V_{\text{нак}}=3,04$ В')
#             r'\frac{-e^{i\pi}}{2^n}$!')
plt.xlabel(r'Ускоряющее напряжение $V_{\text{c}}$, В')
plt.ylabel(r'Анодный ток $I_{\text{а}}$, мА')
plt.grid(True)
